...
import model.json.{JsonInput, JsonOutput, JsonTransaction}
...
class TransactionSpec extends WordSpec with Matchers {
  ...
  "fee only transaction" should {
    val transaction = JsonTransaction(Seq(JsonInput(Some(JsonOutput(Some(30000), Some("3ay"))))), Seq(JsonOutput(Some(26000), Some("3az"))), 0L, 0, 0, "abc", 220, transactionTime)
